...
router.put('/addToCart/', Utils.authenticateToken, (req, res) => {  
  ...
  // add productId to cart field (array - push)
  User.updateOne({
    _id: req.user._id
  }, {
    $push: {
...
router.get('/:id', Utils.authenticateToken, (req, res) => {
  ...
  User.findById(req.params.id).populate('favouriteProducts') .populate('cart')
    .then(user => {
      res.json(user)
    })
    .catch(err => {
